...
scrape_indexpage <- function(url) { # URL = XXV, XXIV, XXIII, ...
  ...
  write_csv(indexpagedata, path = paste0("data/df_", indexpagedata$period[1], ".csv"))
...
get_meta_info <- function(.x){
  ...
  statusbox_text <- statusbox %>% html_text()
  ...
  status <- page %>%
    html_node("#sidebar .rs_1 .verlauf") %>%
    html_attr("src") %>%
    str_extract("[0-9]{2,}")
  ...
  return(tibble::tibble( .id = .x$.id, bill_link, bill_id, proposal_link = .x$proposal_link, proposal_download, iniator, resolution_NR, verfahren, status, statusbox_text))
